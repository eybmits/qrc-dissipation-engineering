...
def collective_loss(n: int, gamma: float, c: Optional[Sequence[float]] = None
                    ) -> List[Tuple[np.ndarray, float]]:
    ...
    coefficient vector is normalised to ``||c||^2 = n`` (so ``Tr(L^dag L)``
    matches the local-loss case)."""
    if c is None:
        c = np.ones(n)
    c = np.asarray(c, float)
    c = c * np.sqrt(n / np.sum(c ** 2))   # ||c||^2 = n
    d = 2 ** n
