...
diamonds and dashed lines mark uniform local. Family-colored markers, bars, and
matching dotted lines identify every design whose mean is better than that
reference in the favorable task direction.
Focused scales retain all non-dephasing seeds in (a--c), with edge labels for
off-scale dephasing. Panels (c,d) use marked piecewise-linear zooms that most
strongly expand \(4.60\)--\(5.15\) parity capacity and
\(0.025\)--\(0.050\) MG-150 MSE, respectively. The outer ranges are compressed,
but every seed remains visible. These mean-based highlights are not
significance tests; rankings are
fixed-\(\mathcal B\), common-point comparisons, not claims of physical
equivalence. Unequal local is profile-only.}
\label{fig:map}
\end{figure*}
...
focus on its most strongly replicated consequence: the local--collective
contrast in readout-accessible recent-input memory.
...
\emph{(a)} Paired STM-capacity gain (collective minus uniform local) under
targeted controls; Hamiltonian variants remain separate. \emph{(b,c)}
Fixed-initialization STM capacity and NARMA-10 NMSE versus
\(N=4,\ldots,8\). \emph{(d)} Delay-resolved STM contribution \(C_\tau\).
Faint marks and lines show paired or seed-level values; large marks, bars, and
