...
\caption{\textbf{Jump-family choice changes the computational task
profile.}
...
Section~\ref{sec:model} defined coupling organization through the jump-family and
rate-profile coordinates. We now traverse the design space of
Fig.~\ref{fig:space} in three steps: first across jump families, then
across rate profiles within a selected family, and finally across both axes
...
We first use the fixed-weight task map to show that coupling organization
changes computation in a task-dependent manner; we then focus on its most
strongly replicated consequence, the local--collective contrast in
readout-accessible recent-input memory.
...
\caption{\textbf{Rate profiles refine the selected jump family.}
...
\caption{\textbf{Finite sampling determines when the jump-family
