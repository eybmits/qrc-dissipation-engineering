...
\emph{(b)} Fixed-initialization STM size sweep for \(N=4,\ldots,8\).
\emph{(c)} Corresponding NARMA-10 size sweep.
\emph{(d)} Delay-resolved STM capacities. Faint marks and lines show paired
effects in (a) and seed-level absolute scores or capacities in (b--d); bars and
