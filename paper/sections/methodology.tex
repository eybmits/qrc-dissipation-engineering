...
memory contrast.}
\emph{(a)} STM gain of collective over local relaxation under four separate
comparisons: fixed assigned weight, matched mean activity, matched relaxation
rate, and independently selected settings. Values above zero favor collective
relaxation. \emph{(b)} Memory from each previous input before and after the
local relaxation rate is matched to the collective relaxation rate. The purple
curve remains higher, so collective relaxation retains more input history even
after that matching. Pale marks show individual reservoir pairs; large symbols,
lines, bars, and bands show means and \(95\%\) intervals.}
