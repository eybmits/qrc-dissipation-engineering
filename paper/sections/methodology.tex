...
the second component of dissipative architecture independently of the jump-family
comparison.
...
uniquely identify the Kossakowski-geometry picture.
...
the separate \(N=6\) sign-balanced replication S. Faint marks show pairs;
large marks and bars show means and \(95\%\) \(t\) intervals (32 and 24 pairs).}
...
coefficient magnitudes, and assigned weight \(\mathcal B=192\). Processor,
input, readout, data, and ridge rule are identical within each pair. All 24
ground/mixed-state audits pass at washout 800; the first six also pass the
four-state audit.

Equal-phase STM is \(13.626\) versus \(11.354\), a paired gain of
\(2.271\,[1.826,2.717]\) with \(24/24\) wins and exact two-sided sign-test
\(p=1.19\times10^{-7}\); every delay-wise mean difference is positive. Equal
phase concentrates the response, but these diagnostic changes are not
detectably associated with the STM gain and are not identified mediators. At
the tested finite sizes, matched rank, spectrum, trace, diagonal weights,
coefficient magnitudes, and assigned weight therefore do not determine
accessible memory: orientation relative to the fixed processor is a
computational coordinate, not a scaling law, universal optimum, or unique
